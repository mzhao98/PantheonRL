...
from influence_envs.rpd import rpd_env
...
def generate_env():
    env = rpd_env(num_actions=2, max_cycles=15)
...
class EgoRPSAgent():
    def __init__(self, env, action_to_one_hot, default_marginal_probability, desired_probability):
        self.ego_model = INFLUENCE_PPO('MlpPolicy', env, verbose=1)

        self.ego_model.set_reward_params(action_to_one_hot, default_marginal_probability, desired_probability)
    ...
    def train_partner_model(self):
        ...
        for epoch in range(num_epochs):
            ...
            # print("out, target", (outputs, trainY))
            # pdb.set_trace()
    ...
    def construct_dataset(self):
        ...
        for i in range(1, len(game_data)-1):
            add_x = []
            if 2 in game_data[i] or 2 in game_data[i+1] or 2 in game_data[i-1]:
                continue
...
def generate_ego(env, action_to_one_hot, default_marginal_probability, desired_probability):
    ego_agent = EgoRPSAgent(env, action_to_one_hot, default_marginal_probability, desired_probability)
    return ego_agent
...
def gen_load(policy_type, location):
    if policy_type == 'INFLUENCE_PPO':
        agent = INFLUENCE_PPO.load(location)
...
def run_test(ego, env, num_episodes, render=False):
    ...
    action_distribution_ego = {0:0, 1:0, 2:0}
    action_distribution_partner = {0: 0, 1: 0, 2: 0}
...
# Train Agent
def train(game_name, desired_probability, num_iterations_per_ep=1000, num_episodes=1):
    env, altenv = generate_env()
    print(f"Environment: {env}; Partner env: {altenv}")

    action_to_one_hot = {0: [1, 0], 1: [0, 1]}
    default_marginal_probability = np.array([0, 0])


    ego_agent = generate_ego(env, action_to_one_hot, default_marginal_probability, desired_probability)
    ...
    for ep in range(num_episodes):
        ...
        partner_prediction_accuracy, partner_action_distribution, ego_action_distribution = test_trained_partner_model(game_name, num_episodes=10)
    ...
    plt.plot(range(num_episodes), partner_prediction_accuracies)
    plt.xlabel("Epoch")
...
def test_trained_partner_model(game_name, num_episodes=10):
    ...
    for game in range(num_episodes):
        ...
        while not done:
            action, ego_action_distr = ego.get_action(obs, False)
            # print("Ego action", number_to_action[action])
            ego_action_distribution[obs.item()] = ego_action_distr
            ...
            ego_action_distr = ego_action_distr[0]
...
def plot_over_time_partner_distribution(over_time_distribution, savename=''):
    # pdb.set_trace()
    for observation in [0,1]:
        plt.figure()
        for timestep in over_time_distribution:
            action_distr = over_time_distribution[timestep][observation]
            action_distr = list(action_distr)
            if len(action_distr)==0:
                continue
            plt.plot(range(len(action_distr)), action_distr)
        plt.legend(list(over_time_distribution.keys()))
        ...
        plt.savefig(f'images/observation_{observation}_'+savename)
        plt.close()

def plot_over_time_ego_distribution(over_time_distribution, savename=''):
    # pdb.set_trace()
    for observation in [0,1]:
        plt.figure()
        for timestep in over_time_distribution:
            action_distr = over_time_distribution[timestep][observation]
            action_distr = list(action_distr[0])
            if len(action_distr)==0:
                continue
            plt.plot(range(len(action_distr)), action_distr)
        plt.legend(list(over_time_distribution.keys()))
...
if __name__ == '__main__':
    game = 'equal_rpd_(full_rew)_influence_exp0_lstm'
    ...
    over_time_partner_action_distribution, over_time_ego_action_distribution = train(game, desired_probability, num_iterations_per_ep, num_episodes)
    print("over_time_partner_action_distribution", over_time_partner_action_distribution)
    print("over_time_ego_action_distribution", over_time_ego_action_distribution)

    plot_over_time_partner_distribution(over_time_partner_action_distribution, savename=f'{game}_partner_distr_over_time')
    plot_over_time_ego_distribution(over_time_ego_action_distribution, savename=f'{game}_ego_distr_over_time')
