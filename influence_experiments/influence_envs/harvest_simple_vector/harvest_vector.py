...
import copy
from .board_w_shoot import Board
...
class raw_env(AECEnv):
    ...
    def __init__(self, max_cycles=400):
        ...
        #     self.agents}
        self.observation_spaces = {i: spaces.Box(low=0, high=1, shape=(27, 1), dtype=np.int8) for i in self.agents}
    ...
    def step(self, action, display=False):
        ...
        if self._agent_selector.is_last():
            ...
            for acting_agent in self.agents:
                ...
                rew, partner_rew = self.board.step_single_action(self.agents.index(acting_agent), selected_action)
                rewards[self.agents.index(acting_agent)] += rew
                rewards[1-self.agents.index(acting_agent)] += partner_rew
