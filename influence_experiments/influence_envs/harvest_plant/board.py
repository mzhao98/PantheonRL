...
class Board():
    def __init__(self):
        ...
        self.action_idx_to_coord = {0: self.NORTH, 1: self.SOUTH, 2:self.EAST, 3:self.WEST, 4:self.CONSUME, 6: self.PLANT, 5:self.STAY}
    ...
    def perform_plant(self, player_index):
        current_player_pos = self.player_positions[player_index]
        current_player_or = self.player_orientations[player_index]
        ...
        if check_apple_obj[0] >= 0 and check_apple_obj[0] < self.grid_w:
            if check_apple_obj[1] >= 0 and check_apple_obj[1] < self.grid_l:
                if self.grid[check_apple_obj] == 0:
                    self.grid[check_apple_obj] = 3
                    self.apple_locations.append(check_apple_obj)
                    self.num_apples_remaining += 1
                    reward = -3
    ...
    def step_single_action(self, p_idx, action):
        ...
        elif action_coord == self.PLANT:
            r = self.perform_plant(player_index=p_idx)
        else:
