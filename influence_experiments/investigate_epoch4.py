...
def run_test(ego, env, max_cyles, num_episodes, number_to_action, render=False):
    ...
    for game in range(num_episodes):
        ...
        while not done:
            ...
            if action_distribution_ego[all_actions[0][0]] ==4:
                print("state", init_obs)
                print()
                # n_apples.append(init_obs[12])
                p_act = [init_obs[m][0] for m in range(6,12)]
                if 1 in p_act:
                    id = p_act.index(1)
                    n_apples.append(id)
    ...
    print("n_apples", np.mean(n_apples))
    print("n_apples", np.std(n_apples))
    # plt.hist(n_apples, bins=[-1, 0.9, 1.9, 2.9, 3.9, 4.9, 5.9, 6.9])
    plt.hist(n_apples, bins=[-0.5, 0.5, 1.5, 2.5, 3.5, 4.5, 5.5, 6.5, 7.5])
    plt.title("Partner Action When Ego Shoots")
    plt.savefig(f'partn_hist.png')
    plt.close()
...
def test(game_params):
    ...
    ep=4
    ego = gen_fixed("INFLUENCE_PPO", f'{game_name}/saved_models/ep{ep}/policy')
    print(f'Ego: {ego}')

    # alt = generate_agent(altenv, args.alt, args.alt_config, args.alt_load)
    # alt =
    alt = gen_fixed("PPO", f'{game_name}/saved_models/ep{ep}/partner_{0}')
...
def main():
    ...
    game = 'harvest_vector_v0_truepartner_pretrained-desired-exp10_load-influence_checkpt-save_15iters-1-10-20'
