...
from stable_baselines3 import INFLUENCE_PPO_HARVEST_V0
# from pettingzoo.classic import rps_v2
# from influence_envs.joint_particle import joint_particle_env
from influence_experiments.influence_envs import harvest_v0
...
def generate_env(max_cycles):
    env = harvest_v0.env(max_cycles=max_cycles)
...
class EgoRPSAgent():
    def __init__(self, env, action_to_one_hot, default_marginal_probability, desired_strategy, partner_params):
        self.ego_model = INFLUENCE_PPO_HARVEST_V0('MlpPolicy', env, device=DEVICE, verbose=1)
...
def gen_load(policy_type, location):
    if policy_type == 'INFLUENCE_PPO':
        agent = INFLUENCE_PPO_HARVEST_V0.load(location)
...
def test_trained_partner_model(game_name, partner_params, action_to_one_hot, max_cycles, num_episodes, number_to_action):
    ...
    for game in range(num_episodes):
        ...
        while not done:
            ...
            # print("Ego action", number_to_action[action])
            # pdb.set_trace()
            ego_observation_as_tuple = np.array2string(env.base_env.observe('player_1')[0])
            partner_observation_as_tuple = np.array2string(env.base_env.observe('player_2')[0])
            ...
            ego_action_t_minus_1.extend([0, 0, 0, 0])
...
if __name__ == '__main__':
    ...
    game = 'harvest_v0_w_shoot_indiv-reward_exp1_15iters_rew-env'
    ...
    game_params = {
        ...
        'number_to_action': {0: 'N', 1: 'S', 2: 'E', 3: 'W', 4: 'C', 5: 'S'},
