...
from stable_baselines3 import INFLUENCE_PPO_CP_RPD
# from pettingzoo.classic import rps_v2
# from influence_envs.joint_particle import joint_particle_env
from influence_experiments.influence_envs import collaborative_particle_v0
...
def generate_env(max_cycles):
    env = collaborative_particle_v0.env(max_cycles=max_cycles)
...
class EgoRPSAgent():
    def __init__(self, env, action_to_one_hot, default_marginal_probability, desired_strategy, partner_params):
        self.ego_model = INFLUENCE_PPO_CP_RPD('MlpPolicy', env, device=DEVICE, verbose=1)
    ...
    def construct_dataset(self):
        ...
        for i in range(1, len(game_data)-1):
            ...
            add_y = state_i2[-1] # Not one hot encoded version
...
def gen_partner(altenv):
    return OnPolicyAgent(PPO(policy='MlpPolicy', env=altenv, device=DEVICE, verbose=1))
...
def gen_load(policy_type, location):
    if policy_type == 'INFLUENCE_PPO':
        agent = INFLUENCE_PPO_CP_RPD.load(location)
...
def run_test(ego, env, max_cyles, num_episodes, number_to_action, render=False):
    ...
    normalized_action_distribution_ego = {k:action_distribution_ego[k]/ego_action_sum for k in action_distribution_ego}

    normalized_partner_distribution_ego = {k:action_distribution_partner[k]/partner_action_sum for k in action_distribution_partner}
...
def train(game_params):
    ...
    for ep in range(num_interaction_episodes):
        ...
        partner_prediction_accuracy, partner_action_distribution, ego_action_distribution = \
            test_trained_partner_model(game_name, partner_params, action_to_one_hot, max_cycles, num_test_games, number_to_action)
...
def test(game_params):
    ...
    game_results, action_distribution_ego, action_distribution_partner = run_test(ego, env, max_cycles, num_eval_games, number_to_action, False)
...
def test_trained_partner_model(game_name, partner_params, action_to_one_hot, max_cycles, num_episodes, number_to_action):
    ...
    for game in range(num_episodes):
        ...
        while not done:
            ...
            # print("Ego action", number_to_action[action])
            # pdb.set_trace()
            ego_observation_as_tuple = tuple(env.base_env.observe('player_1').squeeze(axis=1))
            partner_observation_as_tuple = tuple(env.base_env.observe('player_2').squeeze(axis=1))
            ...
            ego_action_t_minus_1 = action_to_one_hot[action]
    ...
    action_distribution_partner = {action_distribution_partner[elem] / sum(action_distribution_partner.values()) for
                                   elem in
                                   action_distribution_partner}
    print(f"Average Reward: {sum(rewards) * 1.0 / (num_episodes * max_cycles)}")
    print(f"Standard Deviation: {np.std(rewards)}")
    ...
    return prediction_accuracy, partner_action_distribution, ego_action_distribution
...
def plot_over_time_distribution_as_time_series(game, over_time_distribution, number_to_action, number_to_color, savename=''):
    # pdb.set_trace()
    # for observation in [0,1]:
    observation_list = []
    for timestep in over_time_distribution:
    ...
    for observation in observation_list:
        ...
        action_num_to_prob_list = {a: [] for a in number_to_action}
        ...
        for timestep in over_time_distribution:
            ...
            for a in number_to_action:
                action_num_to_prob_list[a].append(np.mean(avg_over_keys[a]))

        for a in number_to_action:
            plt.plot(range(len(action_num_to_prob_list[a])), action_num_to_prob_list[a], color=number_to_color[a], label=number_to_action[a])
...
if __name__ == '__main__':
    ...
    desired_strategy = {
                    (5,0): [0.97, 0.01, 0.01, 0.01],
                    (4,0): [0.97, 0.01, 0.01, 0.01],
                    (3,0): [0.97, 0.01, 0.01, 0.01],
                    (2,0): [0.97, 0.01, 0.01, 0.01],
                    (1,0): [0.29, 0.01, 0.01, 0.49],
                    (0,0): [0.01, 0.01, 0.01, 0.97],

                    (0,1): [0.01, 0.01, 0.01, 0.97],
                    (0,2): [0.01, 0.01, 0.01, 0.97],
                    (0,3): [0.01, 0.01, 0.01, 0.97],
                    (0,4): [0.01, 0.01, 0.01, 0.97],
                    (0,5): [0.01, 0.01, 0.01, 0.97],
                    (0,6): [0.01, 0.01, 0.01, 0.97],
                    (0,7): [0.01, 0.01, 0.01, 0.97],
                    (0,8): [0.01, 0.01, 0.01, 0.97],

                    (1, 1): [0.01, 0.01, 0.01, 0.97],
                    (1, 2): [0.01, 0.01, 0.01, 0.97],
                    (1, 3): [0.01, 0.01, 0.01, 0.97],
                    (1, 4): [0.01, 0.01, 0.01, 0.97],
                    (1, 5): [0.01, 0.01, 0.01, 0.97],
                    (1, 6): [0.01, 0.01, 0.01, 0.97],
                    (1, 7): [0.01, 0.01, 0.01, 0.97],
                    (1, 8): [0.01, 0.01, 0.01, 0.97],

                    (2, 1): [0.01, 0.01, 0.01, 0.97],
                    (2, 2): [0.01, 0.01, 0.01, 0.97],
                    (2, 3): [0.01, 0.01, 0.01, 0.97],
                    (2, 4): [0.01, 0.01, 0.01, 0.97],
                    (2, 5): [0.01, 0.01, 0.01, 0.97],
                    (2, 6): [0.01, 0.01, 0.01, 0.97],
                    (2, 7): [0.01, 0.01, 0.01, 0.97],
                    (2, 8): [0.01, 0.01, 0.01, 0.97],

                    (5, 1): [0.97, 0.01, 0.01, 0.01],
                    (4, 1): [0.97, 0.01, 0.01, 0.01],
                    (3, 1): [0.97, 0.01, 0.01, 0.01],
                    (2, 1): [0.97, 0.01, 0.01, 0.01],
                    (1, 1): [0.29, 0.01, 0.01, 0.49],
                    (0, 1): [0.01, 0.01, 0.01, 0.97],

                    (5, 2): [0.97, 0.01, 0.01, 0.01],
                    (4, 2): [0.97, 0.01, 0.01, 0.01],
                    (3, 2): [0.97, 0.01, 0.01, 0.01],
                    (2, 2): [0.97, 0.01, 0.01, 0.01],
                    (1, 2): [0.29, 0.01, 0.01, 0.49],
                    (0, 2): [0.01, 0.01, 0.01, 0.97],

                    (5, 3): [0.97, 0.01, 0.01, 0.01],
                    (4, 3): [0.97, 0.01, 0.01, 0.01],
                    (3, 3): [0.97, 0.01, 0.01, 0.01],
                    (2, 3): [0.97, 0.01, 0.01, 0.01],
                    (1, 3): [0.29, 0.01, 0.01, 0.49],
                    (0, 3): [0.01, 0.01, 0.01, 0.97],

                    (5, 4): [0.97, 0.01, 0.01, 0.01],
                    (4, 4): [0.97, 0.01, 0.01, 0.01],
                    (3, 4): [0.97, 0.01, 0.01, 0.01],
                    (2, 4): [0.97, 0.01, 0.01, 0.01],
                    (1, 4): [0.29, 0.01, 0.01, 0.49],
                    (0, 4): [0.01, 0.01, 0.01, 0.97],
                    }
    ...
    game_params = {
        ...
        'game_name': game,
        'max_cycles': 50,
        'desired_strategy': desired_strategy_as_str,
        'num_iterations_per_ep': 2000,
        'num_interaction_episodes': 15,
        'num_test_games': 15,
        'action_to_one_hot': {0: [1, 0, 0, 0], 1: [0, 1, 0, 0], 2: [0, 0, 1, 0], 3: [0, 0, 0, 1]},
        'default_marginal_probability': [0, 0, 0, 0],
        'number_to_action': {0: 'N', 1: 'S', 2: 'E', 3: 'W'},
        'number_to_color': {0: 'r', 1: 'g', 2: 'b', 3: 'm'},
        'partner_params':  {
                'learning_rate': 0.01,
                'input_size': 1,
                'hidden_size': 20,
    ...
    plot_over_time_distribution_as_time_series(game, over_time_partner_action_distribution, number_to_action, number_to_color,
