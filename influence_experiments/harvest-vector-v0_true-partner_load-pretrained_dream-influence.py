...
from stable_baselines3 import INFLUENCE_PPO_HARVEST_VECTOR_V1_TRUE_PARTNER_PRETRAINED_DESIRED_DREAM_INFLUENCE as INFLUENCE_PPO
...
class EgoRPSAgent():
    def __init__(self, env, action_to_one_hot, default_marginal_probability, desired_strategy, partner_params):
        self.ego_model = INFLUENCE_PPO('MlpPolicy', env, device=DEVICE, verbose=1)
    ...
    def load_model_from_saved(self, location):
        self.ego_model = INFLUENCE_PPO.load(path=f'{location}/saved_models/policy', env=self.env, device=DEVICE)
...
def train_after_pretrained(game_params):
    ...
    transform_influence_reward_w_weights = game_params['transform_influence_reward_w_weights']
    ...
    for ep in range(num_interaction_episodes):
        ego_ppo.set_partner_model(ego_agent.partner_model, transform_influence_reward, transform_influence_reward_w_weights, DEVICE)
...
def transform_influence_reward_w_weights(env_reward, mi_divergence, mi_guiding_divergence, episode, alpha, beta, gamma):
...
if __name__ == '__main__':
    ...
    game = 'harvest_vector_v0_truepartner_pretrained-desired-exp11_dream-influence'
    ...
    game_params = {
        ...
        'transform_influence_reward_w_weights': transform_influence_reward_w_weights,
        'game_name': game,
        'max_cycles': 50,
