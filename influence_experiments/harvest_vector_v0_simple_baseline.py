...
from stable_baselines3 import INFLUENCE_PPO_HARVEST_VECTOR_V0
...
class EgoRPSAgent():
    def __init__(self, env, action_to_one_hot, default_marginal_probability, desired_strategy, partner_params):
        # self.ego_model = INFLUENCE_PPO_HARVEST_VECTOR_V0('MlpPolicy', env, device=DEVICE, verbose=1)
...
def gen_load(policy_type, location):
    if policy_type == 'INFLUENCE_PPO':
        agent = INFLUENCE_PPO_HARVEST_VECTOR_V0.load(location)
...
def run_test(ego, env, max_cyles, num_episodes, number_to_action, render=False):
    ...
    for game in range(num_episodes):
        ...
        while not done:
            ...
            if action_success[0] == 1:
                action_distribution_ego[all_actions[0][0]] += 1

            if action_success[1] == 1:
...
def train(game_params):
    ...
    plt.ylim(-1, max_y+2)
    plt.title(f'{game_name}: Game Rewards')
    plt.savefig(f'{game_name}/images/game_rewards_0to5.png')
    plt.close()
...
def test_games(game_name, partner_params, action_to_one_hot, max_cycles, num_episodes, number_to_action):
    ...
    for game in range(num_episodes):
        ...
        while not done:
            ...
            partner_observation_as_tuple = tuple(np.squeeze(env.base_env.observe('player_2'), axis=1))
            ...
            game_results[game]['rewards'].append(reward)
            game_results[game]['actions'].append(player_actions)
            game_results[game]['dones'].append(done)
...
if __name__ == '__main__':
    ...
    game = 'harvest_vector_v0_exp5_newspawn_baseline_2onpolicy_15iters_rew-env_repeat1'
