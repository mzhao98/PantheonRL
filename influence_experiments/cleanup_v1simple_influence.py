...
from stable_baselines3 import INFLUENCE_PPO_SIMPLE_CLEANUP_V0
# from pettingzoo.classic import rps_v2
# from influence_envs.joint_particle import joint_particle_env
from influence_experiments.influence_envs import simple_cleanup_v0
...
def generate_env(max_cycles):
    env = simple_cleanup_v0.env(max_cycles=max_cycles)
...
class EgoRPSAgent():
    def __init__(self, env, action_to_one_hot, default_marginal_probability, desired_strategy, partner_params):
        self.ego_model = INFLUENCE_PPO_SIMPLE_CLEANUP_V0('MlpPolicy', env, device=DEVICE, verbose=1)
        # self.ego_model = OnPolicyAgent(PPO(policy='MlpPolicy', env=env, device=DEVICE, verbose=1))
...
def gen_load(policy_type, location):
    if policy_type == 'INFLUENCE_PPO':
        agent = INFLUENCE_PPO_SIMPLE_CLEANUP_V0.load(location)
    ...
    for o in (range(len(observation_list))):
        ...
        for timestep in over_time_distribution:
            ...
            for a in number_to_action:
                action_num_to_prob_list[a].append((np.mean(avg_over_keys[a]) if len(avg_over_keys[a]) > 0 else action_num_to_prob_list[a][-1]))
...
if __name__ == '__main__':
    ...
    game = 'simple_cleanup_v0_exp5_beta100_gamma20_eq-rew_15iters_rew-env-inf-guide'
    ...
    game_params = {
        ...
        'desired_strategy': [0.01, 0.01, 0.49, 0.49],
        ...
        'number_to_action': {0: 'Red', 1: 'Green', 2: 'Blue', 3: 'Yellow'},
