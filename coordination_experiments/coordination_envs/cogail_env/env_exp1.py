...
class gameEnv():
    ...
    def step(self, actions):
        ...
        if self.count_finish > 300:
            ...
            action = [0,0,0,0]
            action[0] = round(actions[0], 1)
            action[1] = round(actions[1], 1)
            ...
            action[2] = round(actions[2], 1)
            action[3] = round(actions[3], 1)
...
def main():
    ...
    env = gameEnv(args)
    print(os.getcwd())
    filename = 'dataset/dataset-continuous-info-act/' + '0.txt'
    ...
    actions = []
    for row in data:
        p1_action = [row[10], row[11]]
        p2_action = [row[12], row[13]]
        action = [row[10], row[11], row[12], row[13]]
