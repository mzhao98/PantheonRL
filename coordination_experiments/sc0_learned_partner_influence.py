...
from stable_baselines3 import SC0_COORD_LEARNED_PARTNER
...
def generate_env(max_cycles, ego_index):
    env = simple_collect_v0.env(max_cycles=max_cycles)

    env = PettingZooAECWrapper(env, ego_ind=ego_index)
...
class HumanLSTM(nn.ModuleList):
    def __init__(self, sequence_len, input_size, hidden_dim, batch_size, output_size):
        ...
        self.lstm_1 = nn.LSTMCell(input_size=input_size, hidden_size=hidden_dim)
    ...
    def init_hidden(self):
        ...
        return (torch.zeros(self.batch_size, self.hidden_dim).to(device=DEVICE),
                torch.zeros(self.batch_size, self.hidden_dim).to(device=DEVICE))


class EgoCollectAgent():
    def __init__(self, env, action_idx_to_encoding, encoding_to_action_idx, default_marginal_probability, partner_params, ego_index):
        self.ego_model = SC0_COORD_LEARNED_PARTNER('MlpPolicy', env, device=DEVICE, verbose=1)
        self.ego_model.set_reward_params(action_idx_to_encoding, default_marginal_probability, ego_index)
        ...
        self.num_classes = self.action_space.n  # 5
        ...
        self.encoding_to_action_idx = encoding_to_action_idx
    ...
    def initialize_partner_model(self):
        ...
        self.partner_model  = HumanLSTM(sequence_len=1, input_size=29, hidden_dim=512, batch_size=2046, output_size=5)
        ...
        # self.optimizer = optim.LBFGS(self.partner_model.parameters(), lr=self.learning_rate)
    ...
    def train_partner_model(self):
        ...
        trainX = Variable(torch.Tensor(np.array(train_x))).to(device=DEVICE)
        trainY = Variable(torch.Tensor(np.array(train_y))).to(device=DEVICE)
        ...
        for epoch in range(self.num_partner_training_epochs):
            ...
            output = self.partner_model(trainX, hc)
            ...
            for t in range(1):
                ...
                # calculate the loss
                loss += self.criterion(output_t, target_t.type(torch.LongTensor))
            print("loss = ", loss)
    ...
    def construct_dataset_old(self):
        ...
        # with open(f'test_past_game_data_{self.eps_index}.pickle', 'wb') as handle:
        #     pickle.dump(self.past_game_data[self.eps_index], handle, protocol=pickle.HIGHEST_PROTOCOL)
        # print("Saved LSTM dataset to", f'test_past_game_data_{self.eps_index}.pickle')

        # print("SIZE OF len(game_data)", len(game_data))
        for i in range(1, len(game_data) - 1):
            ...
            add_x = np.concatenate([state_i1], axis=0)  # length = (14,)
            # add_x = np.expand_dims(np.array([add_x]), axis=0)
            # print("add_x",add_x.shape)
            # pdb.set_trace()
            add_y = self.encoding_to_action_idx[tuple(partner_action_i1)]  # Not one hot encoded version
    ...
    def construct_dataset(self):
        ...
        seq_len = 1
        game_data = self.past_game_data[self.eps_index]
        for i in range(1, len(game_data) - seq_len):
            train_x_seq = []
            train_y_seq = []
            for t in range(seq_len):
                state_i, ego_action_i, partner_action_i, state_i1, rew_i = game_data[i + t]
                _, _, partner_action_i1, _, _ = game_data[i + t + 1]
                train_x_seq.append(state_i)
                #         train_y_seq.append(state_i1)
                train_y_seq.append(self.encoding_to_action_idx[tuple(partner_action_i1)])

            train_x.append(train_x_seq)
            train_y.append(train_y_seq)
...
def generate_ego(env, action_idx_to_encoding, encoding_to_action_idx, default_marginal_probability, partner_params, ego_index):
    ego_agent = EgoCollectAgent(env, action_idx_to_encoding, encoding_to_action_idx, default_marginal_probability, partner_params, ego_index)
    return ego_agent
...
def train(game_params):
    ...
    encoding_to_action_idx = game_params['encoding_to_action_idx']
    ego_index = game_params['ego_index']

    env, altenv = generate_env(max_cycles, ego_index)
    print(f"Environment: {env}; Partner env: {altenv}")

    ego_agent = generate_ego(env, action_idx_to_encoding, encoding_to_action_idx, default_marginal_probability, partner_params, ego_index)
    ...
    for ep in range(num_interaction_episodes):
        ...
        if not os.path.exists(f'{game_name}/saved_models/ep{ep}'):
        ...
        print("Checkpoint 3/5: Save Approximate LSTM Human Partner Models")
        ...
        checkpoint_game_traces = test_games(game_name, action_to_one_hot, max_cycles, num_test_games, number_to_action, ego_index)
...
def test_games(game_name, action_to_one_hot, max_cycles, num_episodes, number_to_action, ego_index):
    env, altenv = generate_env(max_cycles, ego_index)
    print(f"Environment: {env}; Partner env: {altenv}")
...
if __name__ == '__main__':
    game = 'exp_results/sc0_exp2_learned_influence_ppo'
